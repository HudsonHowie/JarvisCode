...
class BasePage(Toplevel, ABC):
    ...
    def display_error(self, msg: Union[Exception, str]):
        ...
        ttk.Label(frame, text=msg, style="basicFrame.TLabel").grid(  # type: ignore
            column=0, row=0)
        ttk.Button(frame, text="Close", command=lambda: tmp.destroy()).grid(
            column=0, row=1)
...
class AdminPromptPage(BasePage):
    ...
    def setup(self, **kwargs):
        ...
        label = ttk.Label(top_frame, text="Enter admin password",
                          style="basicFrame.TLabel").grid(column=0, row=0)
        entry = ttk.Entry(top_frame)
        entry.bind("<Return>", lambda event: self.check_entry(
            entry, entry.get()))
...
class AdminPage(BasePage):
    ...
    def build_button(self, parent_frame: ttk.Frame, index: int, name: str, info: MotorInfo):
        ttk.Label(parent_frame, text=info[4], style="basicFrame.TLabel").grid(
            column=0, row=index + 1)
        l1 = ttk.Entry(parent_frame)
        l1.insert(0, str(info[1]))
        l1.grid(column=1, row=index + 1)
        l1.bind("<Return>", lambda event: self.manager.motors.set_motor_config(
            name, "min", int(l1.get())))

        l2 = ttk.Entry(parent_frame)
        l2.insert(0, str(info[2]))
        l2.grid(column=2, row=index + 1)
        l2.bind("<Return>", lambda event: self.manager.motors.set_motor_config(
            name, "max", int(l2.get())))

        l3 = ttk.Entry(parent_frame)
        l3.insert(0, str(info[3]))
        l3.grid(column=3, row=index + 1)
        l3.bind("<Return>", lambda event: self.manager.motors.set_motor_config(
            name, "home", int(l3.get())))
    ...
    def build_lock(self, parent_frame: ttk.Frame, index: int, name: str):
        ...
        locked_str = StringVar(parent_frame, value=str(
            self.manager.motors.get_motor_locked(name)))
...
class MoveListPage(BasePage):
    ...
    def _add_movelist(self, wanted_added: str, frame_to_search: ttk.Frame):
        ...
        if len(frame_to_search.winfo_children()) > 0:
            ...
            index = -1
        c, r = self._calc_col_and_row(index + 1)
        ml_frame = self.construct_movelist_frame(
            wanted_added, [], frame_to_search)
    ...
    def construct_movelist_frame(self, name: str, moves: List[str], parent_frame: ttk.Frame):
        ...
        val = StringVar()
        insert = EntryWithPlaceholder(
            movelist_frame, placeholder="Add move", textvariable=val)

        insert.grid(column=0, row=1, columnspan=2, sticky="nsew")
        insert.grid_configure(pady=(0, 5))

        insert.bind(
            "<Return>", func=lambda event: add_to_moveset(event, insert))
        insert.bind
        ...
        movebox.bind(
            "<Leave>", func=lambda event: save_moveset(event, movebox))
        ...
        but1 = ttk.Button(button_ml_f, text="Delete",
                          command=lambda: self._delete_movelist(name, parent_frame))
    ...
    def populate_rightside(self, frame: ttk.Frame, frame_to_search: ttk.Frame):
        ...
        del_entry.bind("<Return>", func=lambda event: [self._delete_movelist(
            val.get(), frame_to_search), del_entry.delete(0, END)])
        ...
        add_entry.bind("<Return>", func=lambda event: [self._add_movelist(
            val1.get(), frame_to_search), add_entry.delete(0, END)])
...
class ControlPage(BasePage):
    ...
    def build_other_buttons(self, frame: ttk.Frame):
        ...
        goto_button = ttk.Button(child_frame, text="Goto point",
                                 command=lambda: [goto_entry.delete(0, END), self._set_all_slides_to_point(val.get())])
        ...
        goto_entry.bind(
            "<Return>", func=lambda event: [goto_entry.delete(0, END), self._set_all_slides_to_point(val.get())])

        teach_button = ttk.Button(child_frame, text="Teach point",
                                  command=lambda: [teach_entry.delete(0, END), self._teach_point(val1.get())])
        ...
        teach_entry.bind(
            "<Return>", func=lambda event: [teach_entry.delete(0, END), self._teach_point(val1.get())])

        del_button = ttk.Button(child_frame, text="Delete point",
                                command=lambda: [del_entry.delete(0, END), self._del_point(val2.get())])
        ...
        del_entry.bind(
            "<Return>", func=lambda event: [del_entry.delete(0, END), self._del_point(val2.get())])
...
class PointsPage(BasePage):
    ...
    def setup(self, **kwargs):
        ...
        def build_thing(name: str, info: List[int], x: int, y: int):
            ...
            id1 = canvas.create_text(
                0, y, text=name, activefill="red", fill="white")
            id2 = canvas.create_text(400, y, text=str(
                info), activefill="red", fill="white")
        ...
        maxwidth = max([canvas.bbox(child[1])[2] -
                       canvas.bbox(child[0])[0] for child in ids])

        canvas.config(width=maxwidth + 20, yscrollcommand=bar.set,
...
class PointPage(BasePage):
    ...
    def setup(self, name: str, point: List[int], **kwargs):
        ...
        self.style.configure("pointPage.TLabel", font=(
            None, 22), extends="basicFrame.TLabel")

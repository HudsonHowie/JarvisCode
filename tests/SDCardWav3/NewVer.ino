...
void PlayWav()
{
  static bool ShouldReadFile=true;                       // True if reading file from SD. false if filling I2S buffer
  ...
  if(ShouldReadFile)                                     // Read next chunk of data in from file if needed
  {
    BytesRead=ReadFile(Samples);                      // Read data into our memory buffer, return num bytes read in
    ShouldReadFile=false;                                // Switch to sending the buffer to the I2S
  }
  else
    ShouldReadFile=FillI2SBuffer(Samples,BytesRead);        // We keep calling this routine until it returns true, at which point
